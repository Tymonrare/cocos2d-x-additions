...
def main():
    ...
    project_root = os.path.abspath(os.path.join(build_root, '..'))
    ...
    config.set('DEFAULT', 'builddir', build_root)
    config.set('DEFAULT', 'projectdir', project_root)
    ...
    try:
        tojs_root = build_root
        output_dir = '%s/frameworks/runtime-src/Classes/JSB' % project_root

        cmd_args = {'custom_classes.ini': ('custom_classes', 'jsb_custom_classes')
                    }
        target = 'spidermonkey'
