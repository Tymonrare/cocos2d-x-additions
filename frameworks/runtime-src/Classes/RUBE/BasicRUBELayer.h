...
protected:
	...
	PhysWorldContactListener *m_contactListen;
	...
	//ValueMap worldsConf, objectsConf;
  std::string mapPath;
public:
	//inline const ValueMap* getObjectsConf() { return &objectsConf; };
	...
	virtual bool init(const std::string& map);
	static BasicRUBELayer *create(const std::string& map);
...
inline b2World * BasicRUBELayer::getWorld()
{
	return m_world;
}
