...
b2Body* b2dJson::j2b2Body(b2World* world, Json::Value &bodyValue)
{
    ...
    i = 0;
    Json::Value prop = bodyValue["customProperties"][i++];
    while ( !prop.isNull() ) {
	if(prop["name"].asString() == "script"){
		gm::Body *b = gm::Body::create(body, prop["string"].asString());
		jsBodies.push_back(b);
	}
        prop = bodyValue["fixture"][i++];
    }



	// if (bodyName != "") {
	// 	const ValueMap *objs = GRUBEInst->getObjectsConf();
	// 	if(objs->find(bodyName) != objs->end())
	// 		GConInst->addChild(GObject::create(objs->at(bodyName).asString(), body));
	// }
...
int b2dJson::getAllJSBodies(vector<gm::Body*> &bodies)
{
    bodies.insert( bodies.begin(), jsBodies.begin(), jsBodies.end() );
    return bodies.size();
}
